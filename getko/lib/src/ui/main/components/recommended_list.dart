...
import 'package:getko/src/ui/product/product_page.dart';
...
class RecommendedList extends StatelessWidget {
  ...
  // ];
  List<RecommendProduct> products = [];
  RecommendedList({Key? key, required this.products}) : super(key: key);

  Widget makeHero(index) {
    // print('makeHero => ${products[index].product}');
    String image_src = products[index].product.product_thumbnail;
    if (image_src.contains("http")) {
      return Hero(tag: image_src, child: Image.network(image_src));
    }
    return Hero(tag: image_src, child: Image.asset(image_src));
  }

  @override
  Widget build(BuildContext context) {
    // print('RecommendList => $products');
    return Column(
      children: <Widget>[
        SizedBox(
          height: 20,
                  ...
                  onTap: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (_) => ProductPage(
                            product: Product(
                                products[index].product.product_thumbnail,
                                products[index].product.title,
                                products[index].product.title,
                                products[index].product.regular_price,
                                products[index].product.product_url,
                                products[index].product.product_no))));
                  },
                  child: Container(
